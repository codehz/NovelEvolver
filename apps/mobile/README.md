...
- Bundler: [Rollipop](https://rollipop.dev) (`rollipop.config.ts`), not Metro. RN CLI `start` / `bundle` are overridden via `react-native.config.js`. RN 0.87 removed `rn-get-polyfills`; `patches/react-native@0.87.1.patch` shims it to `@react-native/js-polyfills` until Rollipop supports 0.87 natively.
