...
- Wide project panes use the shared 64dp project-pane header for consistent background and height. Extend its `color.surface` background through the top safe-area inset, and keep the header surface present when a pane has no active document or content.
