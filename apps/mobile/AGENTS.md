...
src/
  ...
  shared/              # theme, overlays, node-compat
...
- Keep the shared header appearance aligned with the settings screens: use the darker `color.surface` background without a bottom divider, `color.accent`, the shared sans font, the standard title size, and `headerShadowVisible={false}`.
- Wide project panes use the shared 64dp project-pane header for consistent background and height. Each column owns its top safe-area padding and `color.surface` background so vertical column borders extend through the status-bar region. Keep the header surface present when a pane has no active document or content.
