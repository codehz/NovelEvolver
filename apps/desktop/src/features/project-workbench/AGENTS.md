...
Composition → domain → session → view kernel / `#app/shared` / `@novelevolver/domain`. Domains must not import other domains' internals; cross-domain traffic uses narrow ports (`openEditorTarget`, `revealInTree`, status item exports) or `lib/` helpers.

Renderer UI: `@novelevolver/domain/*` DTOs. RPC clients: `@novelevolver/desktop-rpc/*` handles. Chrome: `#app/features/project-workbench/chrome` barrel only.
...
- `tree/` = pure view kernel (rows, drag, motion, icons). May depend on `#app/shared` and `@novelevolver/domain` DTO types only. **Must not** import `session/` or any domain.
- `session/changes-feed/` = data plane (feed molecule, tree snapshot/revision, delta apply). May depend on session scopes/handles and `@novelevolver/domain` only. **Must not** import `tree/` or domain UI.
...
Banned hosts: `#app/features/project-workbench/worktree/**`, `#app/features/project-workbench/sidebar/**`, `#app/features/project-workbench/statusbar/**`, `#app/features/project-workbench/state/**`, `#app/features/project-workbench/branch/branch-scopes`.
