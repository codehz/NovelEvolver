...
export const AI_MODELS_STATE_VERSION = 2 as const;

export type AiProviderStateRecord = {
  id: string;
  name: string;
  kind: AiAdapterKind;
  baseUrl: string;
  /** Plaintext secret when known. Missing while `hasApiKey` is still true means opaque/unreadable. */
  apiKey?: string;
  hasApiKey: boolean;
};

export type AiModelStateRecord = {
  id: string;
  providerId: string;
  name: string;
  model: string;
...
export type AiModelsStateData = {
  defaultModelId: string | null;
  providers: AiProviderStateRecord[];
  models: AiModelStateRecord[];
};
...
export const EMPTY_AI_MODELS_STATE: AiModelsStateData = {
  defaultModelId: null,
  providers: [],
  models: [],
};

type AiModelsStateOptions = {
  createId: CreateId;
  data?: AiModelsStateData;
};

export class AiModelsState {
  readonly #createId: CreateId;
  #data: AiModelsStateData;

  constructor(options: AiModelsStateOptions) {
    this.#createId = options.createId;
    this.#data = options.data ?? { ...EMPTY_AI_MODELS_STATE, providers: [], models: [] };
  }
  ...
  serialize(): AiModelsStateData {
    return {
      defaultModelId: this.#data.defaultModelId,
      providers: this.#data.providers.map((entry) => ({ ...entry })),
  ...
  upsertProvider(input: AiProviderConfigWrite): AiModelsSettingsSnapshot {
    ...
    if (input.id) {
      ...
      const nextSecret = applySecretWrite(existing, input.apiKey);
      ...
      const nextSecret = applySecretWrite({ apiKey: undefined, hasApiKey: false }, input.apiKey);
  ...
  upsertModel(input: AiModelConfigWrite): AiModelsSettingsSnapshot {
    ...
    const record: AiModelStateRecord = {
    ...
    if (input.id) {
      ...
      record.id = this.#data.models[index]!.id;
      this.#data.models[index] = record;
    } else {
      this.#data.models.push(record);
    }
...
export function parseAiModelsState(value: unknown): AiModelsStateData {
  if (value === null || typeof value !== "object") {
    return { ...EMPTY_AI_MODELS_STATE, providers: [], models: [] };
  }

  const record = value as Record<string, unknown>;
  const providersRaw = Array.isArray(record.providers) ? record.providers : [];
  const providers: AiProviderStateRecord[] = [];
  ...
  for (const item of providersRaw) {
    ...
    const apiKey =
      typeof entry.apiKey === "string" && entry.apiKey !== "" ? entry.apiKey : undefined;
    const hasApiKey = typeof entry.hasApiKey === "boolean" ? entry.hasApiKey : apiKey !== undefined;
...
function toProviderPublic(record: AiProviderStateRecord): AiProviderConfigPublic {
  return {
    id: record.id,
    name: record.name,
    kind: record.kind,
...
function toModelPublic(record: AiModelStateRecord): AiModelConfigPublic {
  return {
    id: record.id,
...
function applySecretWrite(
  existing: { apiKey?: string; hasApiKey: boolean },
  apiKey: string | undefined,
): { apiKey?: string; hasApiKey: boolean } {
  if (apiKey === undefined) {
    return existing.apiKey
      ? { apiKey: existing.apiKey, hasApiKey: existing.hasApiKey }
