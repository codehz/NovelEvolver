...
type UseAutoCollapseExpandOptions = {
  /** Any step streaming / running / pending / awaiting_user. */
  isLive: boolean;
  /** Change resets user pin/collapse (e.g. work segment id). */
  resetKey: string;
};
...
/**
 * Expand while live (auto), collapse when finished — unless the user pinned open
 * or collapsed during the live window.
 *
 * Rules:
 * - Enter live → force open, clear pin/collapse
 * - Manual close while live → stay closed for this live session
 * - Manual open → pin open until next live entry or resetKey change
 * - Leave live → auto close unless pinned
 */
